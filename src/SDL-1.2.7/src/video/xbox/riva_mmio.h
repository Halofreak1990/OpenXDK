...
#define RIVA_FIFO_FREE(device,hwptr,cnt)                           \
{                                                                  \
   while (device->hidden->FifoFreeCount < (cnt))                          \
	device->hidden->FifoFreeCount = hwptr->FifoFree >> 2;        \
   device->hidden->FifoFreeCount -= (cnt);                                \
}
